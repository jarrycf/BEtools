...
class VizInpOutp_Haplotype(object):
    ...
    def viz_align_haplotype(clss, df, seqid, outcome_colname, seqconfig, conv_nl, predscore_thr=0., return_type='html'):
        ...
        inp_str_lst = ['输入序列'] + [f'{codes[nucl]}{nucl}' for nucl in inp_nucl]
        ...
        for rcounter in range(n_rows):
            ...
            if outcome_colname is not None:
                outp_str_lst = ['{}输出序列\n 概率={:.4f}'.format(codes['prob'], row[outcome_colname])]
            else:
                outp_str_lst = ['{}输出序列'.format(codes['prob'])]
        ...
        pos_str_lst = ['位置编号']+[str(elm) for elm in range(offset_st, offset_end+1)]
        tb.add_row(pos_str_lst)

        ewindow_str_lst = ['可编辑窗口 (*)'] + \
